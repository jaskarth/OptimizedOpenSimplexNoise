 ...
 *
 * Utilizes primitive permutation gradient tables, fused-multiply-add, and method inlining to achieve improved performance on 3d noise.
...
public final class OpenSimplexNoise {
	...
	public OpenSimplexNoise(long seed) {
		...
		for (int i = PSIZE - 1; i >= 0; i--) {
			...
			Grad3 grad3 = GRADIENTS_3D[perm[i]];
			permGrad3[i] = grad3;
			permGrad3_[i * 3 + 0] = grad3.dx;
			permGrad3_[i * 3 + 1] = grad3.dy;
			permGrad3_[i * 3 + 2] = grad3.dz;
	...
	public double sample3_XYBeforeZ(double x, double y, double z)
	{
	...
	public double sample3_XZBeforeY(double x, double y, double z)
	{
		// Combine rotation with skew transform.
		double xz = x + z;
	...
	private double sample3_Base(double xs, double ys, double zs) {
		...
		double value = getValue(xsb, ysb, zsb, xins, yins, zins, inSum, dx0, dy0, dz0);

		return value;
	}

	private double getValue(int xsb, int ysb, int zsb, double xins, double yins, double zins, double inSum, double dx0, double dy0, double dz0) {
		if (inSum <= 1) { // We're inside the tetrahedron (3-Simplex) at (0,0,0)
			return tetrahedron000(xsb, ysb, zsb, xins, yins, zins, inSum, dx0, dy0, dz0);
		} else if (inSum >= 2) { // We're inside the tetrahedron (3-Simplex) at (1,1,1)
			return tetrahedron111(xsb, ysb, zsb, xins, yins, zins, inSum, dx0, dy0, dz0);
		} else { // We're inside the octahedron (Rectified 3-Simplex) in between.
			return octohedron(xsb, ysb, zsb, xins, yins, zins, dx0, dy0, dz0);
		}
	}

	private double octohedron(int xsb, int ysb, int zsb, double xins, double yins, double zins, double dx0, double dy0, double dz0) {
		int xsv_ext0;
		double dz_ext1;
		int zsv_ext0;
		int ysv_ext0;
		...
		aScore = bp1 ? p1 - 1 : 1 - p1;
		aPoint = (byte) (bp1 ? 0x03 : 0x04);
		aIsFurtherSide = bp1;

		// Decide between point (0,1,0) and (1,0,1) as closest

		bScore = bp2 ? p2 - 1 : 1 - p2;
		bPoint = (byte) (bp2 ? 0x05 : 0x02);
		bIsFurtherSide = bp2;
		...
		if (p3 > 1) {
			...
			boolean b1 = aScore <= bScore && aScore < score;
			boolean b2 = aScore > bScore && bScore < score;
			aIsFurtherSide = b1 || aIsFurtherSide;
			bIsFurtherSide = b2 || bIsFurtherSide;
			aPoint = b1 ? 0x06 : aPoint;
			bPoint = b2 ? 0x06 : bPoint;
		} else {
			double score = 1 - p3;
			...
			aIsFurtherSide = !b1 && aIsFurtherSide;
			bIsFurtherSide = !b2 && bIsFurtherSide;
			aPoint = b1 ? 0x01 : aPoint;
			bPoint = b2 ? 0x01 : bPoint;
		}
		...
		if (aIsFurtherSide == bIsFurtherSide) {
			if (aIsFurtherSide) { // Both closest points on (1,1,1) side
				...
				return octohedralContribution(xsb, ysb, zsb, dx0, dy0, dz0, xsv_ext0, dz_ext1, zsv_ext0, ysv_ext0, dx_ext1, ysv_ext1, zsv_ext1, dy_ext1, xsv_ext1, dx_ext0, dy_ext0, dz_ext0);
				...
				int m1 = c & 0x01;
				int m2 = Integer.signum(c & 0x02) | (m1 ^ 1);
				int m4 = (m1 ^ m2);

				dx_ext1 = dx0 - m1 - SQUISH_CONSTANT_3D;
				dy_ext1 = dy0 - m2 - SQUISH_CONSTANT_3D;
				dz_ext1 = dz0 - m4 - SQUISH_CONSTANT_3D;
				xsv_ext1 = xsb + m1;
				ysv_ext1 = ysb + m2;
				zsv_ext1 = zsb + m4;
			...
			byte c1 = aIsFurtherSide ? aPoint : bPoint;
			byte c2 = aIsFurtherSide ? bPoint : aPoint;

			int m1 = c1 & 0x01;
			int m2 = Integer.signum(c1 & 0x02) | (m1 ^ 1);
			int m4 = (m1 ^ m2);

			dx_ext0 = dx0 - m1 - SQUISH_CONSTANT_3D;
			dy_ext0 = dy0 - m2 - SQUISH_CONSTANT_3D;
			dz_ext0 = dz0 - m4 - SQUISH_CONSTANT_3D;
			xsv_ext0 = xsb + m1;
			ysv_ext0 = ysb + m2;
			zsv_ext0 = zsb + m4;
			...
			int cm1 = c2 & 0x01;
			int cm2 = Integer.signum(c2 & 0x02);

			dx_ext1 = fma(cm1, -2, dx_ext1);
			xsv_ext1 += cm1 * 2;

			cm2 &= (cm1 ^ 1);

			dy_ext1 = fma(cm2, -2, dy_ext1);
			ysv_ext1 += cm2 * 2;

			int cm4 = (cm1 | cm2) ^ 1;
			dz_ext1 = fma(cm4, -2, dz_ext1);
			zsv_ext1 += cm4 * 2;
	...
	private double octohedralContribution(int xsb, int ysb, int zsb, double dx0, double dy0, double dz0, int xsv_ext0, double dz_ext1, int zsv_ext0, int ysv_ext0, double dx_ext1, int ysv_ext1, int zsv_ext1, double dy_ext1, int xsv_ext1, double dx_ext0, double dy_ext0, double dz_ext0) {
		double value = 0;
		...
		value += contribution(xsb, ysb, zsb, dx1, dy1, dz1, 1, 0, 0);

		// Contribution (0,1,0)
		double dx2 = dx0  - SQUISH_CONSTANT_3D;
		double dy2 = dy0 - 1 - SQUISH_CONSTANT_3D;
		double dz2 = dz1;
		value += contribution(xsb, ysb, zsb, dx2, dy2, dz2, 0, 1, 0);

		// Contribution (0,0,1)
		double dx3 = dx2;
		double dy3 = dy1;
		double dz3 = dz0 - 1 - SQUISH_CONSTANT_3D;
		value += contribution(xsb, ysb, zsb, dx3, dy3, dz3, 0, 0, 1);

		// Contribution (1,1,0)
		double dx4 = dx0 - 1 - 2 * SQUISH_CONSTANT_3D;
		double dy4 = dy0 - 1 - 2 * SQUISH_CONSTANT_3D;
		double dz4 = dz0  - 2 * SQUISH_CONSTANT_3D;
		value += contribution(xsb, ysb, zsb, dx4, dy4, dz4, 1, 1, 0);

		// Contribution (1,0,1)
		double dx5 = dx4;
		double dy5 = dy0  - 2 * SQUISH_CONSTANT_3D;
		double dz5 = dz0 - 1 - 2 * SQUISH_CONSTANT_3D;
		value += contribution(xsb, ysb, zsb, dx5, dy5, dz5, 1, 0, 1);

		// Contribution (0,1,1)
		double dx6 = dx0  - 2 * SQUISH_CONSTANT_3D;
		double dy6 = dy4;
		double dz6 = dz5;
		value += contribution(xsb, ysb, zsb, dx6, dy6, dz6, 0, 1, 1);

		// First extra vertex
		value += vertex(dx_ext0, dy_ext0, dz_ext0, xsv_ext0, ysv_ext0, zsv_ext0);

		// Second extra vertex
		value += vertex(dx_ext1, dy_ext1, dz_ext1, xsv_ext1, ysv_ext1, zsv_ext1);

		return value;
	}

	private double tetrahedron000(
			int xsb, int ysb, int zsb, double xins, double yins, double zins, double inSum, double dx0, double dy0, double dz0) {
		// Determine which two of (0,0,1), (0,1,0), (1,0,0) are closest.
		double aScore = xins;
		double bScore = yins;

		boolean b1 = aScore >= bScore && zins > bScore;
		boolean b2 = aScore < bScore && zins > aScore;

		bScore = b1 ? zins : yins;
		aScore = b2 ? zins : xins;

		byte aPoint = (byte)(b2 ? 0x04 : 0x01);
		byte bPoint = (byte)(b1 ? 0x04 : 0x02);
		...
		if (wins > aScore || wins > bScore) { // (0,0,0) is one of the closest two tetrahedral vertices.
			...
			return tetrahedron000_A(xsb, ysb, zsb, dx0, dy0, dz0, c);
		} else { // (0,0,0) is not one of the closest two tetrahedral vertices.
			byte c = (byte)(aPoint | bPoint); // Our two extra vertices are determined by the closest two.

			return tetrahedron000_B(xsb, ysb, zsb, dx0, dy0, dz0, c);
		}
	}

	private double tetrahedron000_B(int xsb, int ysb, int zsb, double dx0, double dy0, double dz0, byte c) {
		int zsv_ext1;
		double dy_ext1;
		double dx_ext1;
		double dy_ext0;
		...
		xsv_ext0 = xsb + m1;
		xsv_ext1 = xsb + m1;
		dx_ext0 = dx0 - (m1) - 2 * SQUISH_CONSTANT_3D;
		dx_ext1 = dx0 - m1 - SQUISH_CONSTANT_3D;

		// 0 -> 1
		// 1 -> 0
		m1 ^= 1;
		xsv_ext1 = xsv_ext1 - m1;
		dx_ext1 = dx_ext1 + m1;

		int m2 = Integer.signum(c & 0x02);

		ysv_ext0 = ysb + m2;
		ysv_ext1 = ysb + m2;
		dy_ext0 = dy0 - (m2) - 2 * SQUISH_CONSTANT_3D;
		dy_ext1 = dy0 - m2 - SQUISH_CONSTANT_3D;

		// 0 -> 1
		// 1 -> 0
		m2 ^= 1;
		ysv_ext1 = ysv_ext1 - m2;
		dy_ext1 = dy_ext1 + m2;

		int m4 = Integer.signum(c & 0x04);

		zsv_ext0 = zsb + m4;
		zsv_ext1 = zsb + m4;
		dz_ext0 = dz0 - (m4) - 2 * SQUISH_CONSTANT_3D;
		dz_ext1 = dz0 - m4 - SQUISH_CONSTANT_3D;

		// 0 -> 1
		// 1 -> 0
		m4 ^= 1;
		zsv_ext1 = zsv_ext1 - m4;
		dz_ext1 = dz_ext1 + m4;

		return tetrahedralContribution000(xsb, ysb, zsb, dx0, dy0, dz0,
				dx_ext0, dy_ext0, dz_ext0, dx_ext1, dy_ext1, dz_ext1,
				xsv_ext0, ysv_ext0, zsv_ext0,
				xsv_ext1, ysv_ext1, zsv_ext1
		);
	}

	private double tetrahedron000_A(int xsb, int ysb, int zsb, double dx0, double dy0, double dz0, byte c) {
		int zsv_ext1;
		double dy_ext1;
		double dx_ext1;
		double dy_ext0;
		...
		// 1 -> 0
		mask1 ^= 1;
		xsv_ext0 = xsv_ext0 - mask1;
		dx_ext0 = dx_ext0 + mask1;
		...
		int mask = Integer.signum(c & 0x04);
		...
		// 1 -> 0
		mask ^= 1;
		zsv_ext1 = zsv_ext1 - mask;
		dz_ext1 = dz_ext1 + mask;
	...
	private double tetrahedralContribution000(
			int xsb, int ysb, int zsb, double dx0, double dy0, double dz0,
			double dx_ext0, double dy_ext0, double dz_ext0, double dx_ext1, double dy_ext1, double dz_ext1, int xsv_ext0, int ysv_ext0, int zsv_ext0, int xsv_ext1, int ysv_ext1, int zsv_ext1
	) {
		double value = 0;

		// Contribution (0,0,0)
		value += contribution(xsb, ysb, zsb, dx0, dy0, dz0, 0, 0, 0);
	...
	private double tetrahedron111(
		...
		byte aPoint = (byte) (b2 ? 0x03 : 0x06);
		aScore = b2 ? zins : xins;

		byte bPoint = (byte) (b1 ? 0x03 : 0x05);
		bScore = b1 ? zins : yins;
		...
		if (wins < aScore || wins < bScore) { // (1,1,1) is one of the closest two tetrahedral vertices.
			...
			return tetrahedron111_A(xsb, ysb, zsb, dx0, dy0, dz0, c);

		} else { // (1,1,1) is not one of the closest two tetrahedral vertices.
			byte c = (byte)(aPoint & bPoint); // Our two extra vertices are determined by the closest two.

			return tetrahedron111_B(xsb, ysb, zsb, dx0, dy0, dz0, c);
		}
	}

	private double tetrahedron111_B(int xsb, int ysb, int zsb, double dx0, double dy0, double dz0, byte c) {
		int xsv_ext0;
		int zsv_ext1;
		int ysv_ext1;
		double dy_ext1;
		...
		dx_ext0 = dx0 - m1 - SQUISH_CONSTANT_3D;
		dx_ext1 = dx0 - m1_2 - (2 * SQUISH_CONSTANT_3D);

		dy_ext0 = dy0 - m2 - SQUISH_CONSTANT_3D;
		dy_ext1 = dy0 - m2_2 - (2 * SQUISH_CONSTANT_3D);

		dz_ext0 = dz0 - m4 - SQUISH_CONSTANT_3D;
		dz_ext1 = dz0 - m4_2 - (2 * SQUISH_CONSTANT_3D);

		return tetrahedralContribution111(xsb, ysb, zsb, dx0, dy0, dz0,
				dx_ext0, dy_ext0, dz_ext0, dx_ext1, dy_ext1, dz_ext1,
				xsv_ext0, ysv_ext0, zsv_ext0,
				xsv_ext1, ysv_ext1, zsv_ext1);
	}

	private double tetrahedron111_A(int xsb, int ysb, int zsb, double dx0, double dy0, double dz0, byte c) {
		int xsv_ext0;
		int zsv_ext1;
		int ysv_ext1;
		double dy_ext1;
		...
		xsv_ext0 = xsb + (m1 * 2);
		xsv_ext1 = xsb + m1;
		dx_ext0 = fma(m1, -3, dx0);
		dx_ext1 = fma(m1, -2, dx0);
		...
		zsv_ext0 = zsb + m4;
		zsv_ext1 = zsb + (m4 * 2);
		dz_ext0 = fma(m4, -2, dz0);
		dz_ext1 = fma(m4, -3, dz0);
	...
	private double tetrahedralContribution111(
			int xsb, int ysb, int zsb, double dx0, double dy0, double dz0,
			double dx_ext0, double dy_ext0, double dz_ext0, double dx_ext1, double dy_ext1, double dz_ext1, int xsv_ext0, int ysv_ext0, int zsv_ext0, int xsv_ext1, int ysv_ext1, int zsv_ext1
	) {
		double value = 0;
		...
		double dz3 = dz0 - 2 * SQUISH_CONSTANT_3D;
		value += contribution(xsb, ysb, zsb, dx3, dy3, dz3, 1, 1, 0);

		// Contribution (1,0,1)
		double dx2 = dx3;
		double dy2 = dy0 - 2 * SQUISH_CONSTANT_3D;
		double dz2 = dz0 - 1 - 2 * SQUISH_CONSTANT_3D;
		value += contribution(xsb, ysb, zsb, dx2, dy2, dz2, 1, 0, 1);

		// Contribution (0,1,1)
		double dx1 = dx0  - 2 * SQUISH_CONSTANT_3D;
		double dy1 = dy3;
		double dz1 = dz2;
		value += contribution(xsb, ysb, zsb, dx1, dy1, dz1, 0, 1, 1);

		// Contribution (1,1,1)
		dx0 = dx0 - 2;
		dy0 = dy0 - 2;
		dz0 = dz0 - 2;
		value += contribution(xsb, ysb, zsb, dx0, dy0, dz0, 1, 1, 1);
	...
	private double extraVertices(double dx_ext0, double dy_ext0, double dz_ext0, double dx_ext1, double dy_ext1, double dz_ext1, int xsv_ext0, int ysv_ext0, int zsv_ext0, int xsv_ext1, int ysv_ext1, int zsv_ext1, double value) {
		// First extra vertex
		value += vertex(dx_ext0, dy_ext0, dz_ext0, xsv_ext0, ysv_ext0, zsv_ext0);
	...
	// Use FMA if you target CPUs that support it, otherwise use the direct math version! If your CPU doesn't have FMA, fma() is *wildly* slower!
	private static double fma(double a, double b, double c) {
        return Math.fma(a, b, c);
		// return (a * b) + c;
    }

	private double vertex(double dx_ext1, double dy_ext1, double dz_ext1, int xsv_ext1, int ysv_ext1, int zsv_ext1) {
		// 15 cycles + (1 for xor- throughput of 4, so only takes 1 clock cycle)
		double a1 = fma(-dx_ext1, dx_ext1, 2);
		double a2 = fma(-dy_ext1, dy_ext1, a1);

		double attn_ext1 = fma(-dz_ext1, dz_ext1, a2);

		// 18 cycles
//		double attn_ext1 = 2 - dx_ext1 * dx_ext1 - dy_ext1 * dy_ext1 - dz_ext1 * dz_ext1;
		if (attn_ext1 > 0) {
			return extrapolate(xsv_ext1, ysv_ext1, zsv_ext1, dx_ext1, dy_ext1, dz_ext1) * ((attn_ext1 * attn_ext1) * (attn_ext1 * attn_ext1)) ;
		}

		return 0;
	}

	private double contribution(int xsb, int ysb, int zsb,  double dx1, double dy1, double dz1, int i, int i2, int i3) {
		// Total cycles: 18
//		double adx = dx1 * dx1;
//		double ady = dy1 * dy1;
//		double adz = dz1 * dz1;
//		double attn1 = 2 - adx - ady - adz;

		// Total cycles: 15 + (1 for xor- throughput of 4, so only takes 1 clock cycle)
		double a1 = fma(-dx1, dx1, 2);
		double a2 = fma(-dy1, dy1, a1);
		double attn1 = fma(-dz1, dz1, a2);

		// Can this branch be elided?
		if (attn1 > 0) {
			// Pipeline failure here,  rewrite!
//			attn1 *= attn1;
			// TODO: move adds earlier?
			// Dispatch: 31 cycles
			return extrapolate(xsb + i, ysb + i2, zsb + i3, dx1, dy1, dz1) * ((attn1 * attn1) * (attn1 * attn1));
		}
	...
	private double extrapolate(int xsb, int ysb, int zsb, double dx, double dy, double dz) {
		int idx = perm(xsb, ysb, zsb);

		// Prevent expensive pointer dereference
		int idx3 = idx * 3;
		double adx = permGrad3_[idx3];
		double ady = permGrad3_[idx3 + 1];
		double adz = permGrad3_[idx3 + 2];

		// Total cycles: 15
//		return adx * dx + ady * dy + adz * dz;

		// TODO: more beneficial to use z or x instead of y for better pipelining?
		// Total cycles: 13
		double ady_ = ady * dy;

		double a1 = fma(adx, dx, ady_);
		return fma(adz, dz, a1);
	}

	private int perm(int xsb, int ysb, int zsb) {
		int xm = xsb & PMASK;
		int ym = ysb & PMASK;
		int zm = zsb & PMASK;

		return perm[perm[xm] ^ ym] ^ zm;
	}
	...
	public double sample(double x, double y, double z, double w) {
		...
		return sample4_Base(xs, ys, zs, ws);
	}

	public double sample4_XYBeforeZW(double x, double y, double z, double w) {
	...
	public double sample4_XZBeforeYW(double x, double y, double z, double w) {
	...
	public double sample4_XYZBeforeW(double x, double y, double z, double w) {
	...
	private double sample4_Base(double xs, double ys, double zs, double ws) {
		...
		if (inSum <= 1) { // We're inside the pentachoron (4-Simplex) at (0,0,0,0)
			...
			double dy1 = dy0  - SQUISH_CONSTANT_4D;
			double dz1 = dz0  - SQUISH_CONSTANT_4D;
			double dw1 = dw0  - SQUISH_CONSTANT_4D;
			...
			double dx2 = dx0  - SQUISH_CONSTANT_4D;
			...
			double dz5 = dz0  - 2 * SQUISH_CONSTANT_4D;
			double dw5 = dw0  - 2 * SQUISH_CONSTANT_4D;
			...
			double dy6 = dy0  - 2 * SQUISH_CONSTANT_4D;
			double dz6 = dz0 - 1 - 2 * SQUISH_CONSTANT_4D;
			double dw6 = dw0  - 2 * SQUISH_CONSTANT_4D;
			...
			double dy7 = dy0  - 2 * SQUISH_CONSTANT_4D;
			double dz7 = dz0  - 2 * SQUISH_CONSTANT_4D;
			...
			double dx9 = dx0  - 2 * SQUISH_CONSTANT_4D;
			double dy9 = dy0 - 1 - 2 * SQUISH_CONSTANT_4D;
			double dz9 = dz0  - 2 * SQUISH_CONSTANT_4D;
			...
			double dx10 = dx0  - 2 * SQUISH_CONSTANT_4D;
			double dy10 = dy0  - 2 * SQUISH_CONSTANT_4D;
